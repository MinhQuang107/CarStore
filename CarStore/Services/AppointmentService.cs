...
namespace CarStore.Services
{
    public class AppointmentService : IAppointmentService
    {
        ...
        public AppointmentService(CarStoreContext context)
        {
            _context = context;
        }
		...
		public async Task<Appointment> GetAppointmentByIdAsync(long? id)
        {
            try
            {
                return await _context.Appointment.FirstOrDefaultAsync(c => c.ID == id);
            }
            catch (Exception ex)
            {
                // Xử lý ngoại lệ và ghi nhật ký tại đây nếu cần thiết
        ...
        public async Task<bool> UpdateAppointmentAsync(long id, Appointment updatedAppointment)
        {
            try
            {
                if (id == updatedAppointment.ID)
                {
                    _context.Appointment.Update(updatedAppointment);
                    await _context.SaveChangesAsync();
                    return true;
                }
                return false;
        ...
        //    public async Task<bool> AddAppointmentImagesAsync(ImageCar imageCar)
        //    {
        //        try
        //        {
